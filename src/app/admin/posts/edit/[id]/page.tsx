'use client';



import { useState, useEffect, useCallback, useRef } from 'react';
import { useRouter, useParams } from 'next/navigation';

import { apiService, CategoryTreeNode } from '@/lib/api';

import SuccessNotification from '@/components/SuccessNotification';

import SeoPreview from '@/components/SeoPreview';

import ImageUpload from '@/components/ImageUpload';

import MarkdownEditor from '@/components/MarkdownEditor';
import ContentPreview from '@/components/ContentPreview';
import { htmlToMarkdown } from '@/lib/utils/htmlToMarkdown';
import { formatMarkdown } from '@/lib/utils/formatMarkdown';
import { postImageService } from '@/lib/api/postImageService';
import type { ContentStructure, Locale } from '@/lib/types/postImage';


export default function EditPost() {

  const router = useRouter();

  const params = useParams();

  const postId = params.id as string;



  const [formData, setFormData] = useState({

    title: '',

    content: '',

    excerpt: '',

    // Đa ngôn ngữ
    title_vi: '',
    title_en: '',
    title_ko: '',
    content_vi: '',
    content_en: '',
    content_ko: '',
    description_vi: '',
    description_en: '',
    description_ko: '',
    status: 'draft' as 'published' | 'draft',

    seoTitle: '',

    permalink: '',

    metaDescription: '',

    featuredImageUrl: '',

    canonicalUrl: '',

    breadcrumbTitle: '',

    categoryId: '',

    robotsMeta: {

      index: true,

      nofollow: false,

      noimageindex: false,

      noarchive: false,

      nosnippet: false

    }

  });

  const [categories, setCategories] = useState<CategoryTreeNode[]>([]);

  const [isLoading, setIsLoading] = useState(false);

  const [showSuccess, setShowSuccess] = useState(false);

  const [isLoadingPost, setIsLoadingPost] = useState(true);

  const [errors, setErrors] = useState<{ [key: string]: string }>({});
  const [images, setImages] = useState<Array<{ url?: string, id?: string, file: File, name: string, altText?: string }>>([]);
  const [seoPreview, setSeoPreview] = useState({

    title: '',

    url: '',

    description: '',

    siteName: 'Vinpet'

  });

  const [editStep, setEditStep] = useState<'edit' | 'images'>('edit');

  const [isUploadingImages, setIsUploadingImages] = useState(false);
  const [contentStructure, setContentStructure] = useState<ContentStructure | null>(null);
  const [selectedLocale, setSelectedLocale] = useState<Locale>('vi');
  // Lưu position cho từng ảnh: { imageIndex: position }
  const [imagePositions, setImagePositions] = useState<Record<number, number>>({});



  const loadPost = useCallback(async () => {

    try {

      // Sử dụng getPostFull để lấy đầy đủ các field đa ngôn ngữ
      const post = await (apiService as any).getPostFull?.(postId) || await apiService.getPost(postId);


      // Get category ID from post categories

      const categoryId = post.categories && post.categories.length > 0 ? post.categories[0]._id : '';



      setFormData({

        title: post.title || '',
        content: post.content || '',
        excerpt: post.excerpt || '',
        // Đa ngôn ngữ - lấy từ post object (có thể có hoặc không)
        // Nếu content là HTML, parse về Markdown
        title_vi: (post as any).title_vi || '',
        title_en: (post as any).title_en || '',
        title_ko: (post as any).title_ko || '',
        content_vi: htmlToMarkdown((post as any).content_vi) || htmlToMarkdown(post.content) || '',
        content_en: htmlToMarkdown((post as any).content_en) || '',
        content_ko: htmlToMarkdown((post as any).content_ko) || '',
        description_vi: (post as any).description_vi || '',
        description_en: (post as any).description_en || '',
        description_ko: (post as any).description_ko || '',
        status: post.status,

        seoTitle: post.seoTitle,

        permalink: post.permalink,

        metaDescription: post.metaDescription,

        featuredImageUrl: post.featuredImageUrl,

        canonicalUrl: post.canonicalUrl,

        breadcrumbTitle: post.breadcrumbTitle,

        categoryId: categoryId,

        robotsMeta: post.robotsMeta || {
          index: true,
          nofollow: false,
          noimageindex: false,
          noarchive: false,
          nosnippet: false
        }
      });



      // Set SEO preview

      setSeoPreview({

        title: post.seoTitle || '',
        url: post.permalink || '',
        description: post.metaDescription || '',
        siteName: 'Vinpet'

      });



      // Load existing images if any

      if (post.images && post.images.length > 0) {

        const existingImages = post.images.map((img: any, index: number) => ({
          url: img.url || img.image || '', // Use url or image (base64) or empty string

          id: img.id,

          name: `existing-image-${index}`,

          file: new File([], `existing-image-${index}`) // Placeholder file

        }));

        setImages(existingImages);

      }

    } catch (error) {

      console.error('Error loading post:', error);

      alert('Không thể tải bài viết');

    } finally {

      setIsLoadingPost(false);

    }

  }, [postId]);



  useEffect(() => {

    if (!apiService.isAuthenticated()) {

      router.push('/login');

      return;

    }

    loadCategories();

    loadPost();

  }, [router, postId, loadPost]);

  // Load content structure when entering images step
  useEffect(() => {
    const loadContentStructure = async () => {
      if (editStep === 'images' && postId) {
        try {
          const structure = await postImageService.getContentStructure(postId, selectedLocale);
          setContentStructure(structure);
        } catch (error) {
          console.error('Error loading content structure:', error);
          // Set default structure if API fails
          setContentStructure({
            totalParagraphs: 0,
            availablePositions: [0]
          });
        }
      }
    };
    loadContentStructure();
  }, [editStep, postId, selectedLocale]);

  const loadCategories = async () => {

    try {

      const tree = await apiService.getCategoryTree({

        rootOnly: false,

        includeInactive: false,

      });

      setCategories(tree);

    } catch (error) {

      console.error('Error loading categories:', error);

    }

  };



  // Flatten categories for dropdown

  const flattenCategories = (categories: CategoryTreeNode[], level = 0): Array<{ id: string; name: string; level: number }> => {

    const result: Array<{ id: string; name: string; level: number }> = [];

    categories.forEach(category => {

      const indent = '  '.repeat(level);

      const prefix = level > 0 ? '└─ ' : '';

      const displayName = category.name || category.name_vi || category.name_en || category.slug || 'Unnamed';

      result.push({

        id: category._id,

        name: `${indent}${prefix}${displayName}`,

        level,

      });

      if (category.children && category.children.length > 0) {

        result.push(...flattenCategories(category.children, level + 1));

      }

    });

    return result;

  };



  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {

    const { name, value, type } = e.target;



    if (name.startsWith('robotsMeta.')) {

      const robotKey = name.split('.')[1];

      setFormData(prev => ({

        ...prev,

        robotsMeta: {

          ...(prev.robotsMeta || {
            index: true,
            nofollow: false,
            noimageindex: false,
            noarchive: false,
            nosnippet: false
          }),
          [robotKey]: type === 'checkbox' ? (e.target as HTMLInputElement).checked : value

        }

      }));

    } else {

      setFormData(prev => ({

        ...prev,

        [name]: value,

      }));

    }



    // Update SEO preview

    if (name === 'seoTitle' || name === 'permalink' || name === 'metaDescription') {

      setSeoPreview(prev => ({

        ...prev,

        [name === 'seoTitle' ? 'title' : name === 'permalink' ? 'url' : 'description']: value

      }));

    }



    // Clear error when user starts typing

    if (errors[name]) {

      setErrors(prev => ({

        ...prev,

        [name]: '',

      }));

    }

  };



  // Refs để lưu timeout cho debounce
  const formatTimeouts = useRef<{ [key: string]: NodeJS.Timeout }>({});

  const handleMarkdownChange = (fieldName: string) => (value: string | undefined) => {
    let markdownValue = value || '';
    const prevValue = formData[fieldName as keyof typeof formData] as string || '';

    // Detect paste: nếu content tăng đột ngột (> 100 ký tự), có thể là paste
    const isLikelyPaste = markdownValue.length - prevValue.length > 100;

    // Lưu giá trị ngay lập tức
    setFormData(prev => ({
      ...prev,
      [fieldName]: markdownValue,
    }));

    // Clear timeout cũ nếu có
    if (formatTimeouts.current[fieldName]) {
      clearTimeout(formatTimeouts.current[fieldName]);
    }

    // Format ngay nếu là paste, hoặc sau 1 giây nếu là gõ thường
    const formatDelay = isLikelyPaste ? 100 : 1000;

    formatTimeouts.current[fieldName] = setTimeout(() => {
      let formattedValue = markdownValue;

      // Nếu value có vẻ là HTML (chứa thẻ HTML), tự động parse về Markdown
      if (formattedValue && /<[a-z][\s\S]*>/i.test(formattedValue)) {
        formattedValue = htmlToMarkdown(formattedValue);
      } else if (formattedValue && formattedValue.length > 50) {
        // Format Markdown cho đúng chuẩn
        formattedValue = formatMarkdown(formattedValue);
      }

      // Cập nhật lại với giá trị đã format
      if (formattedValue !== markdownValue) {
        setFormData(prev => ({
          ...prev,
          [fieldName]: formattedValue,
        }));
      }
    }, formatDelay);

    // Clear error when user starts typing
    if (errors[fieldName]) {
      setErrors(prev => ({
        ...prev,
        [fieldName]: '',
      }));
    }
  };

  const handleImagesUploaded = (uploadedImages: Array<{ url?: string, id?: string, file: File, name: string }>) => {
    setImages(uploadedImages);

    // Set the first image as featured image if no featured image is set

    if (uploadedImages.length > 0 && !formData.featuredImageUrl && uploadedImages[0].url) {

      setFormData(prev => ({

        ...prev,

        featuredImageUrl: uploadedImages[0].url!

      }));

    }

  };



  const handleUploadImages = async () => {

    setIsUploadingImages(true);



    try {

      // Upload new images (filter out existing ones)

      const newImages = images.filter(img => img.file.size > 0); // Only files with actual content

      const uploadedImages: Array<{ url: string; id: string; position: number }> = [];



      for (let i = 0; i < newImages.length; i++) {

        const image = newImages[i];

        // Get position for this image (0 for thumbnail, or selected position for content)
        const position = i === 0 ? 0 : (imagePositions[i] || i);

        try {
          const uploadedImage = await postImageService.uploadImage({
            file: image.file,
            postId: postId,
            position: position,
            altText: (image as any).altText || image.name || ''
          });
          uploadedImages.push({
            url: uploadedImage.url,
            id: uploadedImage.id,
            position: uploadedImage.position
          });
          console.log(`Uploaded image ${i + 1}:`, uploadedImage);
        } catch (error) {
          console.error(`Error uploading image ${i + 1}:`, error);
          throw error;
        }
      }



      // Combine existing and new images

      const existingImages = images.filter(img => img.file.size === 0); // Existing images

      const allImages = [...existingImages.map(img => ({ url: img.url!, id: img.id!, position: 0 })), ...uploadedImages];



      // Update post with all images

      const updateData = {

        images: allImages,

        featuredImageUrl: images[0]?.url || ''

      };



      await apiService.updatePost(postId, updateData);



      setShowSuccess(true);

      setTimeout(() => {

        router.push('/admin/posts');

      }, 2000);

    } catch (error) {

      console.error('Error uploading images:', error);

      alert('Có lỗi xảy ra khi upload hình ảnh');

    } finally {

      setIsUploadingImages(false);

    }

  };



  const handleSkipImages = () => {

    setShowSuccess(true);

    setTimeout(() => {

      router.push('/admin/posts');

    }, 2000);

  };



  const validateForm = () => {

    const newErrors: { [key: string]: string } = {};

    // Validation theo API: Cần ít nhất một title và một content (có thể khác ngôn ngữ)
    const hasTitle = !!(formData.title_vi?.trim() || formData.title_en?.trim() || formData.title_ko?.trim() || formData.title?.trim());
    const hasContent = !!(formData.content_vi?.trim() || formData.content_en?.trim() || formData.content_ko?.trim() || formData.content?.trim());

    if (!hasTitle) {
      newErrors.title_vi = 'Vui lòng nhập ít nhất một tiêu đề (tiếng Việt, tiếng Anh hoặc tiếng Hàn)';
    }

    if (!hasContent) {
      newErrors.content_vi = 'Vui lòng nhập ít nhất một nội dung (tiếng Việt, tiếng Anh hoặc tiếng Hàn)';
    }



    setErrors(newErrors);

    return Object.keys(newErrors).length === 0;

  };



  const handleSubmit = async (e: React.FormEvent) => {

    e.preventDefault();



    if (!validateForm()) {

      return;

    }



    setIsLoading(true);



    try {

      // Chỉ gửi các trường cần thiết theo API, loại bỏ các trường không cần thiết
      const postData: Record<string, unknown> = {
        // Đa ngôn ngữ
        title_vi: formData.title_vi || undefined,
        title_en: formData.title_en || undefined,
        title_ko: formData.title_ko || undefined,
        content_vi: formData.content_vi || undefined,
        content_en: formData.content_en || undefined,
        content_ko: formData.content_ko || undefined,
        description_vi: formData.description_vi || undefined,
        description_en: formData.description_en || undefined,
        description_ko: formData.description_ko || undefined,
        // Các trường khác
        status: formData.status,
        seoTitle: formData.seoTitle || undefined,
        // Không gửi permalink khi update để tránh duplicate key error
        // API sẽ tự động generate permalink từ title nếu cần
        // permalink: formData.permalink || undefined,
        metaDescription: formData.metaDescription || undefined,
        featuredImageUrl: formData.featuredImageUrl || undefined,
        canonicalUrl: formData.canonicalUrl || undefined,
        breadcrumbTitle: formData.breadcrumbTitle || undefined,
        robotsMeta: formData.robotsMeta,
        tags: [],

        categories: [],
      };

      // Loại bỏ các trường undefined hoặc rỗng để không gửi lên API
      Object.keys(postData).forEach(key => {
        if (postData[key] === undefined || postData[key] === '') {
          delete postData[key];
        }
      });

      // Đảm bảo KHÔNG gửi permalink khi update (tránh duplicate key error)
      // API sẽ tự động generate permalink từ title nếu cần
      delete postData.permalink;


      console.log('Update post data:', postData);



      // Call API to update post

      await apiService.updatePost(postId, postData);



      // Move to images step

      setEditStep('images');

    } catch (error: any) {
      console.error('Error updating post:', error);

      const errorMessage = error?.response?.data?.message || error?.message || 'Có lỗi xảy ra khi cập nhật bài viết';
      alert(`Lỗi: ${errorMessage}`);
    } finally {

      setIsLoading(false);

    }

  };



  if (isLoadingPost) {

    return (

      <div className="min-h-screen bg-gray-50 flex items-center justify-center">

        <div className="text-center">

          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>

          <p className="text-gray-600">Đang tải bài viết...</p>

        </div>

      </div>

    );

  }



  return (

    <>

      <div className="min-h-screen bg-gray-50">

        <div className="max-w-4xl mx-auto p-6">

          {/* Header */}

          <div className="mb-8">

            <div className="flex items-center justify-between">

              <div>

                <h1 className="text-3xl font-bold text-gray-900">Chỉnh sửa bài viết</h1>

                <p className="text-gray-600 mt-2">Chỉnh sửa và cập nhật bài viết</p>

              </div>

              <button

                onClick={() => router.back()}

                className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"

              >

                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">

                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />

                </svg>

                Quay lại

              </button>

            </div>

          </div>



          {/* Step Indicator */}

          <div className="mb-8">

            <div className="flex items-center justify-center space-x-4">

              <div className={`flex items-center ${editStep === 'edit' ? 'text-blue-600' : 'text-green-600'}`}>

                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${editStep === 'edit' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'
                  }`}>

                  {editStep === 'edit' ? '1' : '✓'}

                </div>

                <span className="ml-2 text-sm font-medium">Chỉnh sửa bài viết</span>

              </div>



              <div className={`w-12 h-0.5 ${editStep === 'images' ? 'bg-green-600' : 'bg-gray-300'}`}></div>



              <div className={`flex items-center ${editStep === 'images' ? 'text-blue-600' : 'text-gray-400'}`}>

                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${editStep === 'images' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'
                  }`}>

                  2

                </div>

                <span className="ml-2 text-sm font-medium">Chỉnh sửa hình ảnh</span>

              </div>

            </div>

          </div>



          {editStep === 'edit' ? (

            <form onSubmit={handleSubmit} className="space-y-6">

              {/* Đa ngôn ngữ */}
              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">

                <div className="px-6 py-4 border-b border-gray-200">

                  <h2 className="text-lg font-medium text-gray-900">Nội dung đa ngôn ngữ</h2>
                  <p className="text-sm text-gray-500 mt-1">Nhập nội dung cho các ngôn ngữ: Tiếng Việt, Tiếng Anh, Tiếng Hàn</p>
                </div>

                <div className="p-6 space-y-6">

                  {/* Title đa ngôn ngữ */}
                  <div className="space-y-4">
                    <h3 className="text-md font-medium text-gray-800">Tiêu đề *</h3>
                    {errors.title_vi && (
                      <div className="bg-red-50 border border-red-200 rounded-md p-3">
                        <p className="text-sm text-red-600">{errors.title_vi}</p>
                      </div>
                    )}
                    <div className="grid grid-cols-1 gap-4">
                      <div>

                        <label htmlFor="title_vi" className="block text-sm font-medium text-gray-700 mb-2">
                          Tiêu đề (Tiếng Việt)
                        </label>

                        <input

                          type="text"

                          id="title_vi"
                          name="title_vi"
                          value={formData.title_vi}
                          onChange={handleInputChange}

                          className={`block w-full px-3 py-2 border ${errors.title_vi ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-blue-500'
                            } rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm`}

                          placeholder="Nhập tiêu đề tiếng Việt..."
                        />
                      </div>
                      <div>
                        <label htmlFor="title_en" className="block text-sm font-medium text-gray-700 mb-2">
                          Title (English)
                        </label>
                        <input
                          type="text"
                          id="title_en"
                          name="title_en"
                          value={formData.title_en}
                          onChange={handleInputChange}
                          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                          placeholder="Enter title in English..."
                        />
                      </div>
                      <div>
                        <label htmlFor="title_ko" className="block text-sm font-medium text-gray-700 mb-2">
                          제목 (한국어)
                        </label>
                        <input
                          type="text"
                          id="title_ko"
                          name="title_ko"
                          value={formData.title_ko}
                          onChange={handleInputChange}
                          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                          placeholder="한국어 제목을 입력하세요..."
                        />
                      </div>
                    </div>
                  </div>



                  {/* Description đa ngôn ngữ */}
                  <div className="space-y-4">
                    <h3 className="text-md font-medium text-gray-800">Mô tả</h3>
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <label htmlFor="description_vi" className="block text-sm font-medium text-gray-700 mb-2">
                          Mô tả (Tiếng Việt)
                        </label>
                        <textarea
                          id="description_vi"
                          name="description_vi"
                          rows={4}
                          value={formData.description_vi}
                          onChange={handleInputChange}
                          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                          placeholder="Nhập mô tả tiếng Việt..."
                        />
                      </div>
                      <div>

                        <label htmlFor="description_en" className="block text-sm font-medium text-gray-700 mb-2">
                          Description (English)
                        </label>

                        <textarea

                          id="description_en"
                          name="description_en"
                          rows={4}
                          value={formData.description_en}
                          onChange={handleInputChange}

                          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                          placeholder="Enter description in English..."
                        />
                      </div>

                      <div>

                        <label htmlFor="description_ko" className="block text-sm font-medium text-gray-700 mb-2">
                          설명 (한국어)
                        </label>

                        <textarea

                          id="description_ko"
                          name="description_ko"
                          rows={4}
                          value={formData.description_ko}
                          onChange={handleInputChange}

                          className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                          placeholder="한국어 설명을 입력하세요..."
                        />
                      </div>
                    </div>
                  </div>

                  {/* Content đa ngôn ngữ */}
                  <div className="space-y-4">
                    <h3 className="text-md font-medium text-gray-800">Nội dung * (Markdown format)</h3>
                    {errors.content_vi && (
                      <div className="bg-red-50 border border-red-200 rounded-md p-3">
                        <p className="text-sm text-red-600">{errors.content_vi}</p>
                      </div>
                    )}
                    <div className="grid grid-cols-1 gap-6">
                      <div>
                        <MarkdownEditor
                          value={formData.content_vi}
                          onChange={handleMarkdownChange('content_vi')}
                          label="Nội dung (Tiếng Việt) - Markdown"
                          placeholder="Nhập nội dung tiếng Việt (Markdown format)..."
                          error={errors.content_vi}
                        />
                      </div>
                      <div>
                        <MarkdownEditor
                          value={formData.content_en}
                          onChange={handleMarkdownChange('content_en')}
                          label="Content (English) - Markdown"
                          placeholder="Enter content in English (Markdown format)..."
                          error={errors.content_en}
                        />
                      </div>
                      <div>
                        <MarkdownEditor
                          value={formData.content_ko}
                          onChange={handleMarkdownChange('content_ko')}
                          label="내용 (한국어) - Markdown"
                          placeholder="한국어 내용을 입력하세요 (Markdown 형식)..."
                          error={errors.content_ko}
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>



              {/* Status */}

              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h2 className="text-lg font-medium text-gray-900">Trạng thái</h2>
                </div>
                <div className="p-6">
                  <div>

                    <label htmlFor="status" className="block text-sm font-medium text-gray-700 mb-2">

                      Trạng thái bài viết
                    </label>

                    <select

                      id="status"

                      name="status"

                      value={formData.status}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                    >

                      <option value="draft">Bản nháp</option>

                      <option value="published">Đã xuất bản</option>

                    </select>

                  </div>

                </div>

              </div>



              {/* SEO Settings */}

              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">

                <div className="px-6 py-4 border-b border-gray-200">

                  <h2 className="text-lg font-medium text-gray-900">Cài đặt SEO</h2>

                </div>

                <div className="p-6 space-y-6">

                  {/* SEO Title */}

                  <div>

                    <label htmlFor="seoTitle" className="block text-sm font-medium text-gray-700 mb-2">

                      SEO Title

                    </label>

                    <input

                      type="text"

                      id="seoTitle"

                      name="seoTitle"

                      value={formData.seoTitle}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                      placeholder="Tiêu đề SEO cho bài viết..."

                    />

                  </div>



                  {/* Permalink */}

                  <div>

                    <label htmlFor="permalink" className="block text-sm font-medium text-gray-700 mb-2">

                      Permalink

                    </label>

                    <input

                      type="text"

                      id="permalink"

                      name="permalink"

                      value={formData.permalink}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                      placeholder="duong-dan-bai-viet"

                    />

                    <p className="mt-1 text-sm text-gray-500">URL thân thiện cho bài viết</p>

                  </div>



                  {/* Meta Description */}

                  <div>

                    <label htmlFor="metaDescription" className="block text-sm font-medium text-gray-700 mb-2">

                      Meta Description

                    </label>

                    <textarea

                      id="metaDescription"

                      name="metaDescription"

                      rows={3}

                      value={formData.metaDescription}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                      placeholder="Mô tả meta cho SEO..."

                    />

                  </div>



                  {/* Canonical URL */}

                  <div>

                    <label htmlFor="canonicalUrl" className="block text-sm font-medium text-gray-700 mb-2">

                      Canonical URL

                    </label>

                    <input

                      type="url"

                      id="canonicalUrl"

                      name="canonicalUrl"

                      value={formData.canonicalUrl}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                      placeholder="https://vinpet.com/..."

                    />

                  </div>



                  {/* Breadcrumb Title */}

                  <div>

                    <label htmlFor="breadcrumbTitle" className="block text-sm font-medium text-gray-700 mb-2">

                      Breadcrumb Title

                    </label>

                    <input

                      type="text"

                      id="breadcrumbTitle"

                      name="breadcrumbTitle"

                      value={formData.breadcrumbTitle}

                      onChange={handleInputChange}

                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"

                      placeholder="Tiêu đề breadcrumb..."

                    />

                  </div>

                </div>

              </div>



              {/* SEO Preview */}

              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">

                <div className="px-6 py-4 border-b border-gray-200">

                  <h2 className="text-lg font-medium text-gray-900">Preview SEO</h2>

                </div>

                <div className="p-6">

                  <SeoPreview

                    title={seoPreview.title}

                    url={seoPreview.url}

                    description={seoPreview.description}

                    siteName={seoPreview.siteName}

                  />

                </div>

              </div>



              {/* Robots Meta */}

              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">

                <div className="px-6 py-4 border-b border-gray-200">

                  <h2 className="text-lg font-medium text-gray-900">Robots Meta</h2>

                </div>

                <div className="p-6 space-y-4">

                  <div className="grid grid-cols-2 gap-4">

                    <label className="flex items-center">

                      <input

                        type="checkbox"

                        name="robotsMeta.index"

                        checked={formData.robotsMeta.index}

                        onChange={handleInputChange}

                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"

                      />

                      <span className="ml-2 text-sm text-gray-700">Index</span>

                    </label>



                    <label className="flex items-center">

                      <input

                        type="checkbox"

                        name="robotsMeta.nofollow"

                        checked={formData.robotsMeta.nofollow}

                        onChange={handleInputChange}

                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"

                      />

                      <span className="ml-2 text-sm text-gray-700">No Follow</span>

                    </label>



                    <label className="flex items-center">

                      <input

                        type="checkbox"

                        name="robotsMeta.noimageindex"

                        checked={formData.robotsMeta.noimageindex}

                        onChange={handleInputChange}

                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"

                      />

                      <span className="ml-2 text-sm text-gray-700">No Image Index</span>

                    </label>



                    <label className="flex items-center">

                      <input

                        type="checkbox"

                        name="robotsMeta.noarchive"

                        checked={formData.robotsMeta.noarchive}

                        onChange={handleInputChange}

                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"

                      />

                      <span className="ml-2 text-sm text-gray-700">No Archive</span>

                    </label>



                    <label className="flex items-center">

                      <input

                        type="checkbox"

                        name="robotsMeta.nosnippet"

                        checked={formData.robotsMeta.nosnippet}

                        onChange={handleInputChange}

                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"

                      />

                      <span className="ml-2 text-sm text-gray-700">No Snippet</span>

                    </label>

                  </div>

                </div>

              </div>



              {/* Submit Button */}

              <div className="flex justify-between">

                <button

                  type="button"

                  onClick={() => setEditStep('images')}

                  className="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"

                >

                  Chỉnh sửa hình ảnh

                </button>



                <div className="flex space-x-4">

                  <button

                    type="button"

                    onClick={() => router.back()}

                    className="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"

                  >

                    Hủy

                  </button>

                  <button

                    type="submit"

                    disabled={isLoading}

                    className="px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"

                  >

                    {isLoading ? 'Đang cập nhật...' : 'Cập nhật bài viết'}

                  </button>

                </div>

              </div>

            </form>

          ) : (

            /* Edit Images Step */

            <div className="space-y-6">

              {/* Info Message */}

              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">

                <div className="flex items-center">

                  <div className="flex-shrink-0">

                    <svg className="h-5 w-5 text-blue-400" fill="currentColor" viewBox="0 0 20 20">

                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />

                    </svg>

                  </div>

                  <div className="ml-3">

                    <h3 className="text-sm font-medium text-blue-800">

                      Chỉnh sửa hình ảnh

                    </h3>

                    <p className="mt-1 text-sm text-blue-700">

                      Thêm, xóa hoặc sắp xếp lại hình ảnh cho bài viết. Hình đầu tiên sẽ được sử dụng làm hình ảnh chính.

                    </p>

                  </div>

                </div>

              </div>



              {/* Image Upload Section */}

              <div className="bg-white shadow-sm border border-gray-200 rounded-lg">

                <div className="px-6 py-4 border-b border-gray-200">

                  <h2 className="text-lg font-medium text-gray-900">Chỉnh sửa hình ảnh</h2>

                  <p className="mt-1 text-sm text-gray-500">

                    Thêm hoặc chỉnh sửa hình ảnh cho bài viết. Hình đầu tiên sẽ được sử dụng làm hình ảnh chính.

                  </p>

                </div>

                <div className="p-6 space-y-6">
                  {/* Locale Selector */}
                  <div className="bg-gray-50 rounded-lg p-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Chọn ngôn ngữ để xem cấu trúc nội dung
                    </label>
                    <select
                      value={selectedLocale}
                      onChange={(e) => setSelectedLocale(e.target.value as Locale)}
                      className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm"
                    >
                      <option value="vi">Tiếng Việt</option>
                      <option value="en">English</option>
                      <option value="ko">한국어</option>
                    </select>
                  </div>

                  {/* Content Structure Display */}
                  {contentStructure && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <h3 className="text-sm font-medium text-blue-900 mb-2">
                        Cấu trúc nội dung ({selectedLocale.toUpperCase()})
                      </h3>
                      <p className="text-sm text-blue-700 mb-2">
                        Tổng số đoạn văn: <span className="font-semibold">{contentStructure.totalParagraphs}</span>
                      </p>
                      <p className="text-sm text-blue-700">
                        Các vị trí có thể chèn ảnh: {contentStructure.availablePositions.length > 0
                          ? contentStructure.availablePositions.join(', ')
                          : '0 (hình ảnh chính)'}
                      </p>
                      <p className="text-xs text-blue-600 mt-2">
                        💡 Vị trí 0 = Hình ảnh chính (thumbnail). Vị trí 1, 2, 3... = Chèn vào giữa các đoạn văn.
                      </p>
                    </div>
                  )}

                  <ImageUpload

                    onImagesUploaded={handleImagesUploaded}

                    currentImages={images}

                    label="Hình ảnh bài viết"

                    multiple={true}

                    postId={postId}
                    contentStructure={contentStructure}
                    imagePositions={imagePositions}
                    content={formData[`content_${selectedLocale}` as keyof typeof formData] as string || ''}
                    onPositionChange={(imageIndex: number, position: number) => {
                      setImagePositions(prev => ({
                        ...prev,
                        [imageIndex]: position
                      }));
                    }}

                  />

                  {/* Content Preview with Images */}
                  {images.length > 0 && (
                    <ContentPreview
                      content={formData[`content_${selectedLocale}` as keyof typeof formData] as string || ''}
                      images={images}
                      imagePositions={imagePositions}
                      locale={selectedLocale}
                    />
                  )}

                </div>

              </div>



              {/* Action Buttons */}

              <div className="flex justify-between">

                <button

                  type="button"

                  onClick={() => setEditStep('edit')}

                  className="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"

                >

                  Quay lại

                </button>



                <div className="flex space-x-4">

                  <button

                    type="button"

                    onClick={handleSkipImages}

                    className="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"

                  >

                    Bỏ qua

                  </button>

                  <button

                    type="button"

                    onClick={handleUploadImages}

                    disabled={isUploadingImages}

                    className="px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"

                  >

                    {isUploadingImages ? 'Đang cập nhật...' : 'Hoàn thành'}

                  </button>

                </div>

              </div>

            </div>

          )}

        </div>

      </div>



      {/* Success Notification */}

      <SuccessNotification

        isVisible={showSuccess}

        title="Cập nhật bài viết thành công!"

        message="Bài viết đã được cập nhật thành công. Đang chuyển hướng..."

        onClose={() => setShowSuccess(false)}

        autoClose={true}

        autoCloseDelay={2000}

      />

    </>

  );

}


